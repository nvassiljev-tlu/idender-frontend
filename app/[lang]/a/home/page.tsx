    ...
    <div className="min-h-screen bg-slate-100 text-slate-800 flex flex-col min-w-[320px]">
      <header className="bg-slate-800 text-white p-4 flex justify-between items-center sticky top-0 z-10 w-full">
        ...
        <div className="relative" ref={profileMenuRef}>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-20" onMouseLeave={() => setShowProfileMenu(false)}>
              <button
                onClick={() => { setShowProfileMenu(false); router.push(`/${lang}/a/profile`); }}
                className="block w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
              >My Profile</button>
              <button
                onClick={() => { setShowProfileMenu(false); router.push(`/${lang}/a/ideas/my`); }}
                className="block w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
              >My Ideas</button>
              {isAdmin && (
                <>
                <button
                  onClick={() => { setShowProfileMenu(false); router.push(`/${lang}/a/ideas/all`); }}
                  className="block w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                >All Ideas</button>

                 <button
                  onClick={() => { setShowProfileMenu(false); router.push(`/${lang}/a/users`); }}
                  className="block w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-100"
                >Users</button>
                </>
              )}
